...
void aleterarContatoBinario(char file[]){
    FILE *arquivo = fopen(file,"rb+"); //lê e escreve em um arquivo binário
    Contato contato; int indice = 1, id;

    if(arquivo){
        printf("Lista de contatos: \n");
        while (fread(&contato,sizeof(Contato),1,arquivo)) {
            printf("%i = %2i/%2i/%4i nome:  %s\n",indice++,contato.dia,contato.mes,contato.ano,contato.nome);
        }
        printf("Digite o índice do contato que deseja alterar: ");
        scanf(" %i",&id);
        id--;
        ...
        if(id >= 0 && id < indice -1){
            ...
            getchar();
            fseek(arquivo, id * sizeof(Contato),SEEK_SET);
            // partindo do início do arquivo, desloca id*sizeof(Contato) bits o ponteiro do arquivo
            // agora apontando para o Contato do índice desejado
...
int main(void){
    ...
    do{
        ...
        switch(opcao){
            ...
            break;
            case SALVAR_BINARIO:
                salvarBinario(agenda,quantidade ,arquivoBin);
            break;
            case CARREGAR_BINARIO:
                quantidade = carregarBinario(agenda,arquivoBin);
            break;
            case EDITAR_BINARIO:
                aleterarContatoBinario(arquivoBin);
            break;
            case SAIR:
                printf("\nAté logo!\n");
            break;
